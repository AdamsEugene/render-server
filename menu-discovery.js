...
const HoverDetector = {
  ...
  styles: {
    title:
      "color: #fff; background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); font-size: 14px; font-weight: bold; padding: 8px 12px; border-radius: 4px;",
    success: "color: #10b981; font-size: 12px; font-weight: bold;",
    info: "color: #3b82f6; font-size: 11px;",
  ...
  selector: {
    generate(element, dom = document) {
      ...
      if (element.className && typeof element.className === "string") {
        const allClasses = element.className.split(" ").filter((c) => c.trim());
...
`;
    ...
    state.mutationObserver = new MutationObserver((mutations) => {
      mutations.forEach((mut) => {
        ...
        const isInsideTarget =
          state.savedTargetElement && state.savedTargetElement.contains(target);
  ...
  showCurrentMenuCandidate() {
    ...
    log.muted(`Menu ${state.currentPathIndex + 1} of ${state.menuPath.length}`);
  },
  ...
  confirmMenuYes() {
    ...
    if (snippet) {
      ...
      log.info("It will test the hover state in the iframe (recordingPlayer1)");
      log.divider();
  ...
  init() {
    ...
    console.clear();
    log.title("HOVER ELEMENT DETECTOR");
    log.muted("v1.5.0 - improved menu detection");
    log.divider();
    ...
    log.muted("Hover over any element for 5s to capture it");
    log.divider();
